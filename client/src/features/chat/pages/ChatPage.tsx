...
const quickActions = [
  "Summarize the current conversation into bullet points.",
  "Extract action items and next steps from this chat.",
  "Translate the last message into clear executive language.",
  "Create a short project brief based on this request.",
];
    ...
    >
      ...
      >
        <div className="min-h-0 flex-1 overflow-y-auto px-4 pt-5 pb-2 custom-scrollbar md:px-6 lg:px-8">
          <div className="mx-auto mb-5 max-w-6xl rounded-[32px] border border-white/[0.08] bg-white/5 p-4 text-sm text-gray-300 shadow-[0_20px_50px_rgba(0,0,0,0.12)] md:p-5">
            <div className="grid gap-4 md:grid-cols-[1.5fr_0.9fr] lg:grid-cols-[2fr_1fr]">
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-white/5 px-3 py-1 text-gray-300">
                    Assistant Replies: {assistantMessageCount}
                  </span>
                </div>
                <p className="text-sm text-gray-400">
                  {currentChatTitle || "New Conversation"} — keep your workspace moving with focused chat tools and enriched AI prompt controls.
                </p>
              </div>
              <div className="grid gap-2 sm:grid-cols-3">
                <button
                  onClick={handleNewChat}
                  className="flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-white transition hover:border-blue-400/30 hover:bg-blue-500/10"
                >
                  <Plus size={16} /> New Chat
                </button>
                <button
                  onClick={handleClearConversation}
                  className="flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-white transition hover:border-amber-400/30 hover:bg-amber-500/10"
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-white transition hover:border-emerald-400/30 hover:bg-emerald-500/10 disabled:cursor-not-allowed disabled:opacity-40"
                >
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[0.8fr_1.2fr] lg:grid-cols-[1fr_1.2fr]">
              <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-[#06080c] px-4 py-3 text-sm text-gray-300">
                <SlidersHorizontal size={16} className="text-blue-300" />
                <span className="min-w-[80px] text-gray-400">Model</span>
                <select
                  value={selectedModel}
                  onChange={(e) => handleModelSelect(e.target.value)}
                  className="min-w-0 flex-1 rounded-xl border border-white/10 bg-[#0c0d12] px-3 py-2 text-sm text-white outline-none transition focus:border-blue-500/40 focus:ring-2 focus:ring-blue-500/10"
                >
              ...
              <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-[#06080c] px-4 py-3 text-sm text-gray-300">
                <span className="min-w-[80px] text-gray-400">Temperature</span>
                <input
                  type="range"
                  min={0.1}
                  max={1}
                  ...
                  className="h-2 w-full cursor-pointer appearance-none rounded-full bg-white/10 accent-blue-400"
                />
                <span className="w-12 text-right text-sm text-gray-200">{temperature.toFixed(1)}</span>
              </label>
            </div>
          </div>
          {messages.length === 0 ? (
            ...
            <div className="mb-3 flex flex-wrap items-center gap-2">
              {quickActions.map((action) => (
                <button
                  ...
                  className="rounded-full border border-white/[0.07] bg-blue-500/10 px-3 py-1.5 text-xs font-semibold text-blue-100 transition hover:border-blue-400/25 hover:bg-blue-500/20 hover:text-white"
                >
                  {action}
                </button>
              ))}
